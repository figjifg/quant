...
def test_c003_monthly_gate_buys_only_on_on_months() -> None:
    dates = pd.to_datetime(["2025-01-30", "2025-01-31", "2025-02-03", "2025-02-28", "2025-03-03"])
    ...
    runs, candidates = run_c003_variants(
        ...
        monthly_regime=_monthly_regime(["2025-01-31", "2025-02-28"], [True, False]),
    ...
    assert candidates["macro_gate_mcap"]["signal_date"].unique().tolist() == [pd.Timestamp("2025-01-31")]
    assert len(runs["macro_gate_mcap"].trades) == 5
    assert runs["macro_gate_mcap"].trades["entry_date"].eq(pd.Timestamp("2025-02-03")).all()
    assert runs["macro_gate_mcap"].trades["exit_date"].eq(pd.Timestamp("2025-03-03")).all()


def test_c003_kospi_and_cash_variants_are_reported() -> None:
    dates = pd.to_datetime(["2025-01-30", "2025-01-31", "2025-02-03"])
    calendar = KRXTradingCalendar(dates)

    runs, _ = run_c003_variants(
        panel=_panel(dates),
        calendar=calendar,
        universe=_universe(dates),
        monthly_regime=_monthly_regime(["2025-01-31"], [False]),
        market_breadth=_market_breadth(dates),
        costs=Costs(commission_bps=0.0, tax_bps_sell=0.0, slippage_bps=0.0),
        segments=((dates[0], dates[-1]),),
        max_positions=5,
    ...
    assert runs["kospi_buy_and_hold"].equity_curve["net_value"].round(6).tolist() == [1.0, 1.01, 1.0201]
    assert runs["cash"].equity_curve["net_value"].tolist() == [1.0, 1.0, 1.0]
...
def _monthly_regime(signal_dates: list[str], regime_on: list[bool]) -> pd.DataFrame:
    return pd.DataFrame(
        {
