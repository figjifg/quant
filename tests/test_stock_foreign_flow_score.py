...
def test_stock_foreign_flow_score_uses_rolling_foreign_amounts_and_within_sector_zscore() -> None:
    signal_date = pd.Timestamp("2025-01-07")
    result = build_stock_foreign_flow_scores(
        _stock_daily(),
    ...
    assert by_ticker.loc["000001", "foreign_flow_20"] == pytest.approx((30.0 + 40.0) / (300.0 + 400.0))
    assert by_ticker.loc["000001", "foreign_flow_60"] == pytest.approx((20.0 + 30.0 + 40.0) / 1000.0)
    assert by_ticker.loc["000001", "raw_stock_foreign_flow_score"] == pytest.approx(a_raw)
    assert by_ticker.loc["000001", "stock_foreign_flow_score"] == pytest.approx((a_raw - mean) / std)
    assert by_ticker.loc["000002", "stock_foreign_flow_score"] == pytest.approx((b_raw - mean) / std)


def test_stock_foreign_flow_score_keeps_missing_stock_flow_as_nan() -> None:
    daily = _stock_daily()
    daily.loc[
    ...
    assert pd.isna(row["raw_stock_foreign_flow_score"])
    assert pd.isna(row["stock_foreign_flow_score"])
    assert bool(row["eligible_for_score"]) is False


def test_stock_foreign_flow_score_at_signal_date_is_unchanged_by_future_rows() -> None:
    signal_date = pd.Timestamp("2025-01-07")
    before = build_stock_foreign_flow_scores(
        _stock_daily(),
        signal_dates=[signal_date],
        value_window=2,
        mcap_window=3,
    ...
    daily = _stock_daily()
    daily.loc[pd.to_datetime(daily["date"]).gt(signal_date), "foreign_net_buy_amount"] = 999999.0
    ...
    after = build_stock_foreign_flow_scores(
        daily,
        signal_dates=[signal_date],
        value_window=2,
        mcap_window=3,
    ...
    pd.testing.assert_series_equal(
        before.sort_values("ticker")["stock_foreign_flow_score"].reset_index(drop=True),
        after.sort_values("ticker")["stock_foreign_flow_score"].reset_index(drop=True),
    )
...
def _stock_daily() -> pd.DataFrame:
    ...
    for ticker, values in foreign.items():
        for date, foreign_value, traded_value in zip(dates, values, traded[ticker], strict=True):
            rows.append(
                {
                    "date": date,
                    "ticker": ticker,
