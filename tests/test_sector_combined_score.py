...
def test_combined_score_can_require_flow_rs_and_breadth() -> None:
    breadth = pd.DataFrame(
        {
            "signal_date": ["2025-03-31", "2025-03-31", "2025-06-30"],
            "sector_code": ["01", "02", "01"],
            "sector_name": ["sector_01", "sector_02", "sector_01"],
            "breadth_score": [2.0, pd.NA, 0.0],
        }
    )

    result = build_sector_combined_scores(_flow_scores(), _rs_scores(), breadth)
    by_sector = result.loc[result["signal_date"].eq(pd.Timestamp("2025-03-31"))].set_index("sector_code")

    assert by_sector.loc["01", "combined_score"] == pytest.approx((1.0 + 0.5 + 2.0) / 3.0)
    assert by_sector.loc["01", "eligible_for_score"]
    assert pd.isna(by_sector.loc["02", "combined_score"])
    assert not bool(by_sector.loc["02", "eligible_for_score"])
...
def test_combined_score_at_signal_date_is_unchanged_by_future_component_rows() -> None:
    ...
    pd.testing.assert_series_equal(
        before.loc[before["signal_date"].eq(signal_date)]
        .sort_values("sector_code")["combined_score"]
        .reset_index(drop=True),
        after.loc[after["signal_date"].eq(signal_date)]
        .sort_values("sector_code")["combined_score"]
        .reset_index(drop=True),
    )
