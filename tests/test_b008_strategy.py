...
from src.strategies.b008_f2_promote import VARIANTS, build_b008_candidates
...
def test_b008_candidates_match_b007_f1_and_f2() -> None:
    features = _features()
    universe = features.loc[:, ["execution_date", "signal_date", "종목코드"]].copy()

    b008_candidates, _, _ = build_b008_candidates(features, universe, min_count=3)
    b007_candidates, _, _ = build_b007_candidates(features, universe, min_count=3)

    pd.testing.assert_frame_equal(
        b008_candidates["f1_baseline"].reset_index(drop=True),
        b007_candidates["f1_baseline"].reset_index(drop=True),
    )
    pd.testing.assert_frame_equal(
        b008_candidates["f2_promote"].reset_index(drop=True),
        b007_candidates["f2_relative_and_absolute"].reset_index(drop=True),
    )


def test_b008_real_panel_reproduces_b007_f2_and_b006_t3(tmp_path: Path) -> None:
    output_dir = tmp_path / "B008_f2_filter_promote"
    config_path = tmp_path / "b008.yaml"
    config_path.write_text(
        f"""experiment_id: B008
panels:
...
""",
    ...
    assert set(B008_VARIANTS_WITH_COSTS()) == set(b008)
    _assert_shared_metrics_equal(b008["f1_baseline"], b006["t3_acceleration"])
    _assert_shared_metrics_equal(b008["cost_0_f1_baseline"], b006["cost_0_t3_acceleration"])
    _assert_shared_metrics_equal(b008["f1_baseline"], b007["f1_baseline"])
    _assert_shared_metrics_equal(b008["cost_0_f1_baseline"], b007["cost_0_f1_baseline"])
    _assert_shared_metrics_equal(b008["f2_promote"], b007["f2_relative_and_absolute"])
    _assert_shared_metrics_equal(b008["cost_0_f2_promote"], b007["cost_0_f2_relative_and_absolute"])

    trades_b008 = pd.read_csv(output_dir / "trades.csv", dtype={"종목코드": "string"})
    trades_b007 = pd.read_csv(
        ROOT / "reports/experiments/B007_filter_role_exploration/trades.csv",
        dtype={"종목코드": "string"},
    )
    f2_b008 = trades_b008.loc[trades_b008["variant"].eq("f2_promote")].drop(columns="variant").reset_index(drop=True)
    f2_b007 = trades_b007.loc[trades_b007["variant"].eq("F2")].drop(columns="variant").reset_index(drop=True)
    shared_columns = [column for column in f2_b007.columns if column in f2_b008.columns]
    pd.testing.assert_frame_equal(f2_b008.loc[:, shared_columns], f2_b007.loc[:, shared_columns])


def B008_VARIANTS_WITH_COSTS() -> tuple[str, ...]:
