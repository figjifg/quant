...
from src.strategies.c007_top20_mcap import run_c007_variants


def test_c007_selects_top_20_by_signal_date_market_cap() -> None:
    ...
    selected = candidates["macro_gate_mcap"].sort_values("rank")
    expected_tickers = [f"{ticker_index:06d}" for ticker_index in range(24, 4, -1)]
    assert set(runs) == {"macro_gate_mcap", "kospi_buy_and_hold", "cash"}
    assert len(selected) == 20
    assert selected["rank"].tolist() == list(range(1, 21))
    assert selected["종목코드"].astype(str).tolist() == expected_tickers
    assert selected["signal_date"].unique().tolist() == [pd.Timestamp("2025-03-31")]
    assert runs["macro_gate_mcap"].trades["entry_date"].tolist() == [pd.Timestamp("2025-04-01")] * 20
