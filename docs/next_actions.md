...
### KR-STATUS-PARSER-NONEXTRACTED-LOCAL-TAXONOMY-A0 — ACTIVE (2026-05-26, via relay; Referee REF-OPEN-005)

Referee directive REF-OPEN-005 (2026-05-26, via relay): open the next local-only
...
- **Scope:** measurement-layer **parser non-extracted local root-cause taxonomy**
  only; suspension_related + resumption_related; the **711 parser non-extracted
  rows** (511 no_label_match + 200 label_no_value), including 11 no_label + 7
  label_no_value correction overlaps. **Existing local artifacts + cached bodies
  (read-only) ONLY — NO downloads, NO API, NO body repair, NO parser feature
  expansion / code change / extraction upgrade, NO candidate/body rerun, NO
  downstream wiring, NO C2/C3, NO event-log finalization, NO executable-status
  table, NO strategy/performance/execution.**
- **Goal:** classify WHY extraction failed per row (root-cause classes), preserving
  original parse_status (511/200 split) and ALL fail-closed flags. NOT parser
  expansion — taxonomy only. No effective_date extracted/assigned. Produce a
  design-only (NOT approved) parser-backlog candidate table.
- **Outputs under** `reports/experiments/measurement_A0/KR_STATUS_PARSER_NONEXTRACTED_LOCAL_TAXONOMY_A0/`.
