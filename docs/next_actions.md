...
### KR-STATUS-RESIDUAL-BLOCKER-REGISTER-A0 — ACTIVE (2026-05-26, via relay; Referee REF-OPEN-004)

Referee directive REF-OPEN-004 (2026-05-26, via relay): open the next local-only
...
- **Scope:** measurement-layer **residual blocker register** (local artifact
  consolidation) only; suspension_related + resumption_related status rows;
  **existing local artifacts ONLY — NO downloads, NO API, NO body repair, NO parser
  feature expansion, NO candidate/body rerun, NO downstream wiring, NO C2/C3, NO
  all-event event-log finalization, NO executable-status final table, NO
  strategy/performance/execution.**
- **Goal:** consolidate all prior residual blockers into ONE fail-closed, row-level
  register keyed by rcept_no — 42 universe zip_unparseable + 511 no_label_match +
  200 label_no_value + all 166 correction rows (with accepted action classes);
  reconcile the 39 correction-zip ⊂ 42 universe-zip overlap; assign blocker tags;
  produce overlap matrix + fail-closed policy table + manual-review packet. This is
  NOT an event log, NOT an executable-status table, NOT downstream wiring.
- **Outputs under** `reports/experiments/measurement_A0/KR_STATUS_RESIDUAL_BLOCKER_REGISTER_A0/`.
