...
### KR-STATUS-CORRECTION-LINKAGE-FULL-UNIVERSE-VALIDATION-A0 — ACTIVE (2026-05-26, via relay; Referee REF-OPEN-002)

Referee directive REF-OPEN-002 (2026-05-26, via relay): open the next local-only
...
- **Scope:** measurement-layer correction-linkage **full-universe** validation only;
  suspension_related + resumption_related; correction-flagged rows + candidate
  originals; **existing local artifacts + cached bodies only — NO downloads, NO API,
  NO data acquisition, NO parser feature expansion, NO downstream wiring, NO
  strategy/performance/execution.**
- **Goal:** run the accepted Pass-3 body-confirmation gate across **all 166**
  in-scope correction rows (Pass 3 ran the body gate on the ~72-row sample only;
  body coverage has since expanded to ~98.3%, so all 166 now have cached bodies:
  127 html_inline + 39 zip_unparseable). Reconcile against prior Pass-3 counts
  (166 = 35 high_validated / 42 medium / 18 low / 71 no_link / 0 rejected at
  universe level); explain class movement; classify every row with explicit
  evidence state + manual-review status.
- **Outputs under** `reports/experiments/measurement_A0/KR_STATUS_CORRECTION_LINKAGE_FULL_UNIVERSE_VALIDATION_A0/`.
- **Autonomy:** user-authorized autonomous continuation for measurement-layer
  data-cleaning (local-only); this does NOT authorize downloads/API/strategy/exec.
