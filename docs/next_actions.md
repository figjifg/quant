...
## Active — KR-EXECUTABLE-STATUS-PRE2018-EXTENSION-A0 (Referee verdict 2026-05-24)

**Scope**: Measurement-layer executable-status source extension. Extend or attempt to
extend executable-status coverage 2010-01-01 → 2017-12-31. **No strategy testing. No
performance diagnostics. No execution simulation. No production / paper / P08 / live
/ shadow.**

**Reason**: Executable-status coverage A0 closed PARTIAL with `pre_2018_status_coverage_gap`
as its primary historical defect. OPENDART pblntfty=I (거래소공시) tested to confirm
pre-2018 data IS available via the same API used by Round 4 S3.
...
- `KR_LISTED_UNIVERSE_COVERAGE_A0/listed_lifecycle_coverage_table.csv`

**8 allowed task groups**: source feasibility check / acquisition attempt / taxonomy
normalisation / 2010-2017 panel reconciliation / gap closure assessment / coverage
...
`PRE2018_STATUS_RECONCILED_BUT_EXECUTION_STILL_CLOSED` / `READY_FOR_NEXT_A0_REVIEW`.
Do NOT mark execution simulation open. Do NOT mark strategy testing open. Do NOT mark
any card strategy-ready.

**Required outputs (12)**: see `pre2018_referee_lock.md`.

**Output 경로**: `reports/experiments/measurement_A0/KR_EXECUTABLE_STATUS_PRE2018_EXTENSION_A0/`
