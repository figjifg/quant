...
## Active — KR-STATUS-CORRECTION-LINKAGE-A0 (Referee verdict 2026-05-25; PASS 3 REQUIRED)

**State (2026-05-25)**: **PASS 3 REQUIRED / CORRECTION_LINKAGE_REQUIRES_MORE_WORK**.
Pass 2 (commit `565f0d3`) reduced no_link from 123 to 70 (via raw pool + cross-cat
matrix + window expansion + rcept_dt parser fix), but sample link rate remained
55.1% (below 60% bar) and wrong-candidate risk = 12/80 = 15%. Validation-quality
pass required before any close decision.

**Pass 1 + Pass 2 artifacts preserved** (do NOT overwrite):
- Pass 1: 12 base-name outputs.
- Pass 2: 12 `pass2_*` outputs.
- Pass 3 outputs go to same directory with `pass3_` prefix.

**Pass 3 objective**: narrower than Pass 2 — focus on validation quality.
- Reduce wrong-candidate risk.
- Resolve or better classify the remaining 70 no_link rows.
- Distinguish "no link because source truly absent" vs "no link because scoring is
  too weak".
- Decide whether sample link rate above 60% is achievable without unacceptable
  false positives.
- Decide whether correction linkage can become sample-validated or must remain
  manual-review-only.
