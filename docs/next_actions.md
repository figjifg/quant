...
### KR-STATUS-MEASUREMENT-A0-HANDOFF-STATE-MANIFEST-A0 — ACTIVE (2026-05-26, via relay; Referee REF-OPEN-012)

Referee directive REF-OPEN-012 (2026-05-26, via relay): open the next local-only
measurement-layer data-cleaning phase. Follows the now-closed
KR-STATUS-MANUAL-REVIEW-WORKLIST-VIEWS-A0. (Active was empty before opening; only this
phase is now Active.)

- **Status: ACTIVE** (Executor pass in progress; Executor does NOT self-close;
  directive: no CLOSE_NOTE this phase).
- **Scope:** local **handoff-state manifest** only — consolidate the accepted closed
  measurement_A0 state (REF-CLOSE-007 .. 011) into a compact, deterministic handoff
  index for future Referee/Executor sessions. **NOT source recovery, NOT parser
  design, NOT manual adjudication, NOT validation, NOT an event log, NOT an
  executable-status table, NOT strategy/execution.** Existing local CSV/MD only; no
  new data; no edits to closed artifacts; changes NO underlying data and grants NO
  action authority.
- **Goal:** state manifest (phase / accepted commits / status / key outputs / locked
  counts / hard-lock summary / next-action boundary); key-locks summary
  (count/set/field/packet/worklist); output inventory with sha256 checksum metadata
  (stdlib); worklist state re-check (862 / 862 unique / WL-00001..WL-00862 / no
  forbidden outcome columns); "does NOT authorize" boundary notes; defect ledger.
- **Outputs under** `reports/experiments/measurement_A0/KR_STATUS_MEASUREMENT_A0_HANDOFF_STATE_MANIFEST_A0/`.
