...
### BX01-KOSPI200-MISSING-TIER1-XLSX-ACQUIRE-A0 (opened 2026-05-31; initial pass 2026-05-31)

- **Phase pre-reg + addendum:** carry-forward from prior BX01 phases +
  `reports/experiments/BX01_KOSPI200_index_event_source_A0/missing_tier1_xlsx_acquire_initial_pass.md`
- **Decision channel:** user → claude-session "외부 네트워크 허용할테니 내가 해야한다는
  작업은 대신 진행해 줘. 다음으로 진행할 작업은 레프리랑 의논해서 진행하고 자율 모드로
  계속 진행해 줘." → Referee selected option E first → directive `ask_0010.md`.
- **Status: INITIAL PASS COMPLETE, awaiting Referee gate verdict.**
- **Outcome:** 0/4 missing Tier 1 cycles (2018-06/2019-06/2020-06/2020-12) acquired.
  All public paths (OTP / direct static / Wayback / FinanceDataReader / wiki / news
  mirror) blocked or empty. One documented OTP probe (bbsSeq=555) returned
  identical 0-byte result as BX01-source-A0 → route stopped + recorded.
- **Recommended gate:** `BACKLOG_MISSING_TIER1_STRUCTURED_SOURCE_GAP`
  (Referee's expected wording from ask_0010).
- **Next per directive:** "If this phase closes cleanly, the natural next candidate
  is C, Tier 2/3 parse." Decision after this phase's gate sign-off.
