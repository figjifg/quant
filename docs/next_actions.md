...
### W000-DATA-INFRASTRUCTURE-ACQUISITION — ACTIVE (2026-05-26, 사용자 명시 결정, 자율 데이터모드 + 다운로드 사전승인)

사용자 결정 (2026-05-26): P08_IEF30 ops/live-readiness 는 문서상 완료 (U000-U003 / V000 /
N006 / Z000 / W001 v1 모두 COMPLETE, blocker 없음). 남은 데이터 작업 = **W000 데이터 인프라
백로그**를 **자율모드로 진행**하며, **외부 데이터 취득(다운로드/API)을 사전승인**한다.

- **Status: ACTIVE** (executor 자율 진행). 계획 = `docs/w000_acquisition_plan.md`.
- **승인 범위:** W000 6개 항목 데이터 취득을 위한 다운로드/API 사용 사전승인.
- **하드룰 (자율로 못 넘김):** `data/raw/` · `research_input_data/` 원본 수정 금지 (취득물은
  `data/acquired/**` gitignored + 커밋된 inventory/lineage); P08_IEF30 weight 변경 금지;
  strategy/backtest/execution/paper/live 주장 금지; **W000 Hard Rule** = 데이터 확보가
