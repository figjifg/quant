...
### KR-STATUS-CORRECTION-RESIDUAL-LOCAL-ADJUDICATION-A0 — ACTIVE (2026-05-26, via relay; Referee REF-OPEN-003)

Referee directive REF-OPEN-003 (2026-05-26, via relay): open the next local-only
...
- **Scope:** measurement-layer **residual adjudication packet** only;
  suspension_related + resumption_related correction rows; **existing local
  artifacts ONLY — NO downloads, NO API, NO body repair, NO parser feature
  expansion, NO downstream supersession wiring, NO C2/C3, NO strategy/performance/
  execution.** This is NOT residual-source recovery.
- **Goal:** package the 166 correction rows from the closed full-universe phase into
  a row-level residual-action ledger — assign each a local residual action class
  (mapped back to the accepted 5-tier 17/52/7/73/17); detail the 17
  rejected_wrong_candidate; produce a recovery-REQUIREMENTS (not recovery) ledger
  for the 39 zip_unparseable; root-cause + next-action for no_link/medium/low;
  a manual-review packet (no human-validation / authoritative claim).
- **Outputs under** `reports/experiments/measurement_A0/KR_STATUS_CORRECTION_RESIDUAL_LOCAL_ADJUDICATION_A0/`.
- **Autonomy:** user-authorized local measurement-layer data-cleaning (local-only);
  does NOT authorize downloads/API/strategy/exec.
