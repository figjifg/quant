...
### BX01-KOSPI200-INDEX-EVENT-SOURCE-A0 (opened 2026-05-28)

- **Phase pre-reg:** `research/experiments/BX01_KOSPI200_index_event_source_A0.md`
- **Decision channel:** user → Codex (Referee) direct (verbatim quote `"bx01로 진행하자.
  외부 작업은 익스큐터한테 부탁해서 진행하면 될거야. 외부 네트워크 접근도 허용할께."`) +
  parallel claude-session confirmation. Phase opened via Referee `ask_0006.md` → Executor
  acknowledged in `claude_reply_03.md`.
- **Diagnostic-only.** Source acquisition + event-table A0 validation only. Does **not**
  open backtest / strategy / P08 / production / paper-live / parser / measurement-layer
  reopening / DART body parser / closed-family reopening / MSCI / FTSE / KOSDAQ150 scope.
- **Allowed:** external network access for public license-respecting KRX official sources
  (KOSPI200 regular + special review notices 2010-2026); raw artifacts under
  `data/acquired/bx01_kospi200_index_event_source_a0/` with sha256 manifests; parsed
  event table with 11 fields; PIT + near-miss feasibility assessment; reconciliation
  report; initial-pass report to Referee for gate verdict.
- **Forbidden:** no backtest / return / run-up / edge calc; no strategy artifacts; no
  paywall bypass / credential use; no same-day execution assumption from announcement;
  no use of later constituent history as event source of truth; no row labeled
  strategy/executable/approved/production/paper-ready; no self-close; no downstream
  phase opening.
- **Next:** Initial-pass report bridged to Referee for gate verdict
  (`PASS_TO_DIAGNOSTIC_BACKTEST_DESIGN` / `BACKLOG_SOURCE_GAP` / `FAIL_CLOSED`). Any next
  step after that requires a separate user + Referee decision.
