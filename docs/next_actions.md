...
### KR-STATUS-RESIDUAL-MANUAL-REVIEW-PACKET-CONSOLIDATION-A0 — ACTIVE (2026-05-26, via relay; Referee REF-OPEN-010)

Referee directive REF-OPEN-010 (2026-05-26, via relay): open the next local-only
...
- **Scope:** local **manual-review packet consolidation** only — a single
  human-review TRIAGE/INDEX over all 862 blocker-register rows, keyed by rcept_no,
  using the already-accepted count/row-key/fail-closed locks. **Existing local CSV/MD
  ONLY — NO new data, NO edits to closed artifacts, NO fixes, NO source recovery, NO
  parser design, NO downloads/API/credentials/body-repair/parser-change/rerun/
  candidate-rerun/body-confirmation-rerun/downstream-wiring/C2-C3/event-log/
  executable-status-table/strategy/execution.** Triage index only — NOT a fix, NOT
  an event log, NOT an executable-status table.
- **Goal:** packet keyed by rcept_no (preserve 862 exactly); attach parse_status /
  taxonomy root cause / source-recovery class / correction confidence + action /
  fail-closed flags; assign a conservative review bucket per row; count
  reconciliation by bucket + by source blocker; examples per bucket; confirm prior
  audit sentinels clean (rowkey mismatch = NONE, fail-closed violation = NONE);
  packet-build defect ledger. Human-review-only and fail-closed.
- **Outputs under** `reports/experiments/measurement_A0/KR_STATUS_RESIDUAL_MANUAL_REVIEW_PACKET_CONSOLIDATION_A0/`.
