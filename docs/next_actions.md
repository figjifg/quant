...
## Active — KR-STATUS-CORRECTION-LINKAGE-A0 (Referee verdict 2026-05-25; PASS 2 REQUIRED)

**State (2026-05-25)**: **PASS 2 REQUIRED / CORRECTION_LINKAGE_REQUIRES_MORE_WORK**.
Pass 1 (initial pass commit `3d09033`) accepted as pass-1 evidence but NOT
close-ready: 53.3% sample link rate, 123/166 no_link, 74% in-scope corrections with
no candidate at default 180-day window. Candidate-search expansion required.

**Pass 1 artifacts preserved** (do NOT overwrite):
- `correction_candidate_links.csv`, `manual_link_validation_sample.csv`,
  `correction_linkage_defect_ledger.csv`, and other 9 outputs.
- Pass 2 outputs go to the same directory with `pass2_` prefix.

**Pass 2 objective**: diagnose & reduce the 123 no_link cases. Test causes:
