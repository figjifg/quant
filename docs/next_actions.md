...
### KR-STATUS-PARSER-UNHANDLED-FORMAT-DESIGN-PROOF-A0 — ACTIVE (2026-05-26, via relay; Referee verdict, user-approved local-only design-proof)

Referee directive (2026-05-26, via relay): open a LOCAL-ONLY unhandled-format
parser-design PROOF phase for the 23 `label_present_but_value_in_unhandled_format` rows,
authorized by the user's explicit decision. **NOT** download/API approval, **NOT**
source-recovery approval, **NOT** parser-change approval, **NOT** manual adjudication
approval. Follows the now-closed KR-STATUS-PARSER-TABLE-CONTEXT-DESIGN-PROOF-A0.

- **Status: ACTIVE** (Executor pass complete this commit; does NOT self-close; no
  CLOSE_NOTE this pass; do NOT move to Closed/Frozen). Awaiting Referee review.
- **Scope:** exactly the **23 rows** with prior taxonomy
  ...
  derived from the accepted closed feasibility/taxonomy artifacts. Read-only inspect
  cached local HTML/body for those 23 only; no download/reacquire/repair/overwrite.
- **Goal:** PROOF-ONLY design evidence — for each row, the immediate value text after the
  matched date label, value pattern class, design-proof bucket, false-positive risk,
  required guardrails, future-approval label; hypothetical/proof-only candidate text
  (NOT effective_date/parsed_date). Proves whether a future parser extension could safely
  handle the value or whether it must stay fail-closed.
...
- **Outputs under** `reports/experiments/measurement_A0/KR_STATUS_PARSER_UNHANDLED_FORMAT_DESIGN_PROOF_A0/`.
