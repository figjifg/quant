...
### KR-STATUS-PARSER-TABLE-CONTEXT-DESIGN-PROOF-A0 — ACTIVE (2026-05-26, via relay; Referee verdict, user-approved local-only design-proof)

Referee directive (2026-05-26, via relay): open a LOCAL-ONLY table-context parser-design
PROOF phase, authorized by the user's explicit decision. **NOT** download/API approval,
**NOT** source-recovery approval, **NOT** parser-change approval, **NOT** manual
adjudication approval. Follows the now-closed KR-STATUS-PARSER-NONEXTRACTED-FEASIBILITY-A0.

- **Status: ACTIVE** (Executor pass in progress; does NOT self-close; no CLOSE_NOTE this
  pass; do NOT move to Closed/Frozen).
- **Scope:** the **170 rows** with feasibility_bucket=needs_table_context_design (prior
  taxonomy label_present_but_attachment_or_table_context_required; table context only,
  0 attachment), derived from the accepted closed feasibility/taxonomy artifacts.
  Read-only inspect cached local HTML/body for those 170 only; do NOT download/reacquire/
  repair/overwrite anything.
- **Goal:** PROOF-ONLY design evidence — table structure profile, label-cell / candidate
  value-cell location, hypothetical candidate date text (named hypothetical/proof-only,
  NOT effective_date/parsed_date), ambiguity class, false-positive risk, required
  guardrails; design-proof buckets; future-approval labels. Proves whether
  table/structure-aware extraction is design-feasible + how to guard FP — does NOT
  implement or promise extraction.
- **Forbidden:** downloads/API/credentials/source-recovery/body-repair; parser code/rule/
  version change; NO edits under src/parsers/; parser/candidate/body rerun changing
  accepted outputs; manual adjudication/validation/approval; marking any row parsed/
  recovered/safe/validated/approved/executable/authoritative/ready; effective-date
  accept/finalize; rcept_dt as effective date; downstream/C2-C3/event-log/
  executable-status-table/strategy/execution. Every row fail-closed. CSV LF; git
  show --check must pass.
- **Outputs under** `reports/experiments/measurement_A0/KR_STATUS_PARSER_TABLE_CONTEXT_DESIGN_PROOF_A0/`.
