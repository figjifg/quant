...
- **Recommended gate to Referee:** `BACKLOG_EFFECTIVE_DT_PARTIAL_SECONDARY_BASIS`
  (Referee's own expected wording) — 9 cycles filled in the new
  `effective_dt_rulebook_derived` column (canonical `effective_dt` remains blank
  per Referee rule) is material progress, but **6 residual blockers** prevent PASS:
  3 carry-forward (4 skeleton Tier 1 cycles / Class B regular-special conflation /
  Tier 2/3 deferred) + 3 new (primary KRX rulebook access blocked / primary KRX
  derivatives spec access blocked / 2026-06 calendar coverage cutoff).
