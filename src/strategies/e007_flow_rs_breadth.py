...
VARIANTS = ("factor_macro_gate_flow_rs_breadth_top3", "kospi_buy_and_hold", "cash")
...
def build_e007_flow_rs_breadth_top_sector_candidates(
    *,
    panel: pd.DataFrame,
...
def build_e007_sector_selection_log(candidates: pd.DataFrame, combined_scores: pd.DataFrame) -> pd.DataFrame:
...
def empty_candidates_by_variant() -> dict[str, pd.DataFrame]:
    return {
        "factor_macro_gate_flow_rs_breadth_top3": _candidate_frame(),
...
def _top_sector_ranks(combined_scores: pd.DataFrame, *, top_n: int) -> pd.DataFrame:
    scores = combined_scores.loc[
        :, ["signal_date", "sector_code", "flow_score", "rs_score", "breadth_score", "combined_score"]
    ].copy()
    ...
    scores["sector_breadth_score"] = pd.to_numeric(scores["breadth_score"], errors="coerce")
    scores["sector_combined_score"] = pd.to_numeric(scores["combined_score"], errors="coerce")
    scores = scores.dropna(
        subset=["sector_flow_score", "sector_rs_score", "sector_breadth_score", "sector_combined_score"]
    )
    scores = scores.sort_values(["signal_date", "sector_combined_score", "sector_code"], ascending=[True, False, True])
