...
VARIANTS = ("factor_macro_gate_flow_top3", "kospi_buy_and_hold", "cash")
...
def build_e004_flow_top_sector_candidates(
    *,
    panel: pd.DataFrame,
    ...
    _require_columns(flow_scores, ("signal_date", "sector_code", "sector_name", "flow_score"), "flow_scores")
    ...
    ranked = _eligible_with_sector_mcap(panel, eligible)
    score_ranks = _top_sector_ranks(flow_scores, top_n=len(top_sector_counts))
    ...
    selected["fnv_5"] = 0.0
    selected["inv_5"] = 0.0
    selected["combined_flow_5"] = selected["sector_flow_score"]
...
def build_e004_sector_selection_log(candidates: pd.DataFrame, flow_scores: pd.DataFrame) -> pd.DataFrame:
    score_ranks = _top_sector_ranks(flow_scores, top_n=3)
    rows = []
...
def empty_candidates_by_variant() -> dict[str, pd.DataFrame]:
    return {
        "factor_macro_gate_flow_top3": _candidate_frame(),
        "kospi_buy_and_hold": _empty_candidates(),
        "cash": _empty_candidates(),
    }


def _eligible_with_sector_mcap(panel: pd.DataFrame, eligible: pd.DataFrame) -> pd.DataFrame:
    ...
    sector = _sector_from_panel(panel)
    ranked = ranked.merge(sector, on=["signal_date", "종목코드"], how="inner", validate="one_to_one")
    ranked = ranked.loc[ranked["market_cap"].gt(0)].copy()
    return ranked


def _sector_from_panel(panel: pd.DataFrame) -> pd.DataFrame:
    candidates = [
        ("sector_code", "sector_name"),
        ("섹터코드", "섹터명"),
        ("final_sector_code", "final_sector_name"),
    ]
    for code_column, name_column in candidates:
        if code_column in panel.columns and name_column in panel.columns:
            data = panel.loc[:, ["날짜", "종목코드", code_column, name_column]].copy()
            data["signal_date"] = pd.to_datetime(data["날짜"], errors="raise").dt.normalize()
            data["종목코드"] = data["종목코드"].astype("string").str.zfill(6)
            data["sector_code"] = data[code_column].astype("string").str.zfill(2)
            data["sector_name"] = data[name_column].astype("string")
            return data.loc[:, ["signal_date", "종목코드", "sector_code", "sector_name"]].drop_duplicates()
    raise ValueError("panel must include sector_code/sector_name columns for E004 selection.")


def _top_sector_ranks(flow_scores: pd.DataFrame, *, top_n: int) -> pd.DataFrame:
    scores = flow_scores.loc[:, ["signal_date", "sector_code", "sector_name", "flow_score"]].copy()
    ...
    scores = scores.dropna(subset=["sector_flow_score"]).sort_values(
        ["signal_date", "sector_flow_score", "sector_code"], ascending=[True, False, True]
    )
    scores["sector_rank"] = scores.groupby("signal_date").cumcount() + 1
    return scores.loc[scores["sector_rank"].le(top_n), ["signal_date", "sector_code", "sector_name", "sector_flow_score", "sector_rank"]]
