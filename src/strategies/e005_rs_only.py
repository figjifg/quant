...
VARIANTS = ("factor_macro_gate_rs_top3", "kospi_buy_and_hold", "cash")
...
def build_e005_rs_top_sector_candidates(
    *,
    panel: pd.DataFrame,
    ...
    _require_columns(rs_scores, ("signal_date", "sector_code", "sector_name", "rs_score"), "rs_scores")
    ...
    score_ranks = _top_sector_ranks(rs_scores, top_n=len(top_sector_counts))
    ...
    selected["fnv_5"] = 0.0
    selected["inv_5"] = 0.0
    selected["combined_flow_5"] = selected["sector_rs_score"]
...
def build_e005_sector_selection_log(candidates: pd.DataFrame, rs_scores: pd.DataFrame) -> pd.DataFrame:
    score_ranks = _top_sector_ranks(rs_scores, top_n=3)
    sector_names = (
        rs_scores.loc[:, ["sector_code", "sector_name"]]
...
def empty_candidates_by_variant() -> dict[str, pd.DataFrame]:
    return {
        "factor_macro_gate_rs_top3": _candidate_frame(),
...
def _sector_from_panel(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    raise ValueError("panel must include sector_code/sector_name columns for E005 selection.")


def _top_sector_ranks(rs_scores: pd.DataFrame, *, top_n: int) -> pd.DataFrame:
    scores = rs_scores.loc[:, ["signal_date", "sector_code", "sector_name", "rs_score"]].copy()
    ...
    scores = scores.dropna(subset=["sector_rs_score"]).sort_values(
        ["signal_date", "sector_rs_score", "sector_code"], ascending=[True, False, True]
    )
    scores["sector_rank"] = scores.groupby("signal_date").cumcount() + 1
    return scores.loc[
        scores["sector_rank"].le(top_n),
        ["signal_date", "sector_code", "sector_rs_score", "sector_rank"],
    ]
