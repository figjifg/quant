...
def build_f005_e014_selection_universe(
    *,
    panel: pd.DataFrame,
    ...
    _require_columns(
        stock_scores,
        ("signal_date", "ticker", "stock_liquidity_score", "raw_stock_liquidity_score"),
        "stock_scores",
    )
    ...
    return ranked.dropna(subset=["stock_liquidity_score"]).reset_index(drop=True)


def build_f005_liquidity_e014_candidates(
    *,
    panel: pd.DataFrame,
    universe: pd.DataFrame,
    quarterly_regime: pd.DataFrame,
    ...
    ranked = build_f005_e014_selection_universe(
        panel=panel,
        universe=universe,
    ...
    for signal_date, date_group in ranked.groupby("signal_date", sort=True):
        ...
        for sector_rank, count in enumerate(top_sector_counts, start=1):
            sector_group = date_group.loc[date_group["sector_rank"].eq(sector_rank)].sort_values(
                ["stock_liquidity_score", "market_cap", "종목코드"],
    ...
    selected["fnv_5"] = 0.0
    selected["inv_5"] = 0.0
    selected["combined_flow_5"] = selected["stock_liquidity_score"]
