...
_CANDIDATE_COLUMNS = [
    ...
    "stock_liquidity_score_universe",
    "raw_stock_liquidity_score",
]


def build_f005_d013_direct_score_universe(
    *,
    panel: pd.DataFrame,
    ...
    _require_columns(
        stock_scores,
        ("signal_date", "ticker", "sector_code", "sector_name", "stock_liquidity_score", "raw_stock_liquidity_score"),
        "stock_scores",
    )
    ...
    ranked["stock_liquidity_score_universe"] = ranked.groupby("signal_date", group_keys=False)[
        "raw_stock_liquidity_score"
    ].transform(_zscore)
    return ranked.dropna(subset=["stock_liquidity_score_universe"]).reset_index(drop=True)


def build_f005_liquidity_d013_direct_candidates(
    *,
    panel: pd.DataFrame,
    universe: pd.DataFrame,
    quarterly_regime: pd.DataFrame,
    ...
    ranked = build_f005_d013_direct_score_universe(
        panel=panel,
        universe=universe,
    ...
    for signal_date, date_group in ranked.groupby("signal_date", sort=True):
        selected = date_group.sort_values(
            ["stock_liquidity_score_universe", "market_cap", "종목코드"],
    ...
    selected["fnv_5"] = 0.0
    selected["inv_5"] = 0.0
    selected["combined_flow_5"] = selected["stock_liquidity_score_universe"]
