...
from src.roles.filters import (
    ...
    filter_persistence_4_of_5,
    filter_relative_AND_absolute_positive,
)
from src.roles.rankings import rank_by_combined_flow_5
from src.roles.triggers import trigger_acceleration


VARIANTS = ("f1_baseline", "f2_relative_and_absolute", "f3_persistence_4_of_5")
FILTER_CANDIDATES = ("flow_sign_both_positive", "relative_AND_absolute_positive", "persistence_4_of_5")
FEATURE_COLUMNS = (
    "날짜",
    "execution_date",
    "signal_date",
...
def build_b007_candidates(
    flow_features: pd.DataFrame,
    universe: pd.DataFrame,
    *,
    min_count: int = 30,
) -> tuple[dict[str, pd.DataFrame], dict[str, object], pd.DataFrame]:
    """Build B007's three fixed-filter candidates on the promoted T3 carrier."""
    ...
    candidates = {
        ...
        "f2_relative_and_absolute": rank_by_combined_flow_5(trigger_acceleration(f2_filtered, flow_features)),
        "f3_persistence_4_of_5": rank_by_combined_flow_5(trigger_acceleration(f3_filtered, flow_features)),
    }
    return candidates, exit_signal_reversal(flow_features), relative_features


def run_b007_variants(
    *,
    panel: pd.DataFrame,
    ...
    """Run B007's three pre-registered filter variants."""
    candidates, exit_kwargs, relative_features = build_b007_candidates(
        flow_features,
        universe,
        min_count=min_count,
    )
