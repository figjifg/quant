...
def build_stock_foreign_flow_scores(
    ...
    *,
    ...
    """Compute stock-level foreign flow scores with signal-date data only.

    Missing stock-level foreign flow inputs are preserved as NaN. A stock is
    score-eligible only when both rolling components are fully observed.
    """
    ...
    stocks["foreign_net_buy_amount_20d"] = by_stock["foreign_net_buy_amount"].transform(
    ...
    stocks["foreign_net_buy_amount_60d"] = by_stock["foreign_net_buy_amount"].transform(
        lambda values: values.rolling(mcap_window, min_periods=mcap_window).sum()
    )
    stocks["foreign_flow_20"] = stocks["foreign_net_buy_amount_20d"] / stocks["traded_value_20d"]
    stocks["foreign_flow_60"] = stocks["foreign_net_buy_amount_60d"] / stocks["market_cap"]
    invalid_denominator = stocks["traded_value_20d"].le(0) | stocks["market_cap"].le(0)
    stocks.loc[invalid_denominator, ["foreign_flow_20", "foreign_flow_60"]] = pd.NA
    stocks["raw_stock_foreign_flow_score"] = stocks[["foreign_flow_20", "foreign_flow_60"]].mean(
        axis=1,
        skipna=False,
    )
    ...
    valid_raw = scores["raw_stock_foreign_flow_score"].notna()
    ...
    scores.loc[~scores["eligible_for_score"], "raw_stock_foreign_flow_score"] = pd.NA
    scores["stock_foreign_flow_score"] = scores.groupby(["date", "sector_code"], group_keys=False)[
...
def _normalize_stock_daily(stock_daily: pd.DataFrame) -> pd.DataFrame:
    ...
    for column in ("market_cap", "traded_value", "foreign_net_buy_amount", "daily_return"):
